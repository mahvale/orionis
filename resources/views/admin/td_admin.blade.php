    ...
    <h1 class="mb-4">Liste des tds</h1>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if($td->image)
                    <img src="{{ asset('/uploads/td/' . $td->image) }}" alt="{{ $td->title }}" width="100">
                    @else
                    Aucun fichier
                    @endif
                </td>
                ...
                <td>
                    ...
                    <button class="btn btn-warning btn-sm" data-toggle="modal" data-target="#edittdModal{{ $td->id }}">
                        Modifier
                    </button>

                    <!-- Formulaire de suppression -->
                    <form action="{{ route('td_admin.destroy', $td->id) }}" method="POST" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="edittdModalLabel">Modifier td</h5>
                            <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form action="{{ route('td_admin.update', $td->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    @if($td->image)
                                        <small>Image actuelle : <a href="{{ asset('tds/' . $td->image) }}" target="_blank">{{ $td->image }}</a></small>
                                    @endif
                                </div>
...
<div class="modal fade" id="addtdModal" tabindex="-1" aria-labelledby="addtdModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addtdModalLabel">Ajouter un td</h5>
                <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('td_admin.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="title">Titre du td</label>
